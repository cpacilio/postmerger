...
def final_mass(
    ...
    if not aligned_spins:
        if not np.all(spin1 >= 0):
            raise ValueError(
                "spin1 must be non-negative. If you want spin1 to point in the negative direction, set beta!=0 or aligend_spins=True."
            )
        if not np.all(spin2 >= 0):
            raise ValueError(
                "spin2 must be non-negatve. If you want spin2 to point in the negative direction, set gamma!=0 or aligned_spins=True."
            )
    elif aligned_spins:
    ...
    if method == "B12":
        ...
        E_rad = (1 - E_isco(a_tot)) * eta + 4 * eta**2 * (
            4 * p0 + 16 * p1 * a_tot * (a_tot + 1) + E_isco(a_tot) - 1
        )
        m_rad = E_rad * m_tot
        m_fin = m_tot - m_rad
...
def final_spin(
    ...
    if method == "H16":
        ...
        kappas = [[-3.82, -1.2019, -1.20764], [3.79245, 1.18385, 4.90494]]
        ...
        kappas = [
            [2 * 3**0.5, -0.085, 0.102, -1.355, -0.868],
    ...
    a_xy_2 = (
        spin1**2
        + q**4 * spin2**2
        + 2 * q**2 * spin1 * spin2 * np.cos(alpha)
        - (spin1 * np.cos(beta) + q**2 * spin2 * np.cos(gamma)) ** 2
    ) / (1 + q) ** 4
    a_fin = (a_fin_z**2 + a_xy_2) ** 0.5

    if return_angle:
        theta_fin = np.arccos(np.clip(a_fin_z / a_fin, -1.0, 1.0))
        return np.clip(a_fin, -1.0, 1.0), theta_fin
    else:
        return np.clip(a_fin, -1.0, 1.0)
...
def qnm_Kerr(mass, spin, mode, prograde=1, qnm_method="interp", SI_units=True):
    ...
    if (
        hasattr(mass, "__len__")
        and (hasattr(spin, "__len__"))
        and len(mass) != len(spin)
    ):
    ...
    if qnm_method == "L18":
        ...
        ##
        A = {
            (2, 2, 0): np.array([1.0, 1.5578, 1.9510, 2.0997, 1.4109, 0.4106]),
            (2, 2, 1): np.array([1.0, 1.8709, 2.7192, 3.0565, 2.0531, 0.5955]),
            (3, 3, 0): np.array([1.5, 2.0957, 2.4696, 2.6655, 1.7584, 0.4991]),
            (3, 3, 1): np.array([1.5, 2.3391, 3.1399, 3.5916, 2.4490, 0.7004]),
            (4, 4, 0): np.array([2.0, 2.6589, 2.9783, 3.2184, 2.1276, 0.6034]),
            (4, 3, 0): np.array([1.5, 0.2050, 3.1033, 4.2361, 3.0289, 0.9084]),
            (5, 5, 0): np.array([2.5, 3.2405, 3.4906, 3.7470, 2.4725, 0.6994]),
            (3, 2, 0): np.array(
                [1.0225, 0.2473, 1.7047, 0.9460, 1.5319, 2.2805, 0.9215]
            ),
            (2, 1, 0): np.array(
                [0.5891, 0.1890, 1.1501, 6.0459, 11.1263, 9.3471, 3.0384]
            ),
        }
        ##
        B = {
            (2, 2, 0): np.array([0.0, 2.9031, 5.9210, 2.7606, 5.9143, 2.7952]),
            (2, 2, 1): np.array([0.0, 2.5112, 5.4250, 2.2857, 5.4862, 2.4225]),
            (3, 3, 0): np.array([0.0, 2.9650, 5.9967, 2.8176, 5.9327, 2.7817]),
            (3, 3, 1): np.array([0.0, 2.6497, 5.5525, 2.3472, 5.4435, 2.2830]),
            (4, 4, 0): np.array([0.0, 3.0028, 6.0510, 2.8775, 5.9897, 2.8300]),
            (4, 3, 0): np.array([0.0, 0.5953, 3.0162, 6.0388, 2.8262, 5.9152]),
            (5, 5, 0): np.array([0.0, 3.0279, 6.0888, 2.9212, 6.0365, 2.8766]),
            (3, 2, 0): np.array(
                [0.0049, 0.6653, 3.1383, 0.1632, 5.7036, 2.6852, 5.8417]
            ),
            (2, 1, 0): np.array(
                [0.0435, 2.2899, 5.8101, 2.7420, 5.8441, 2.6694, 5.7915]
            ),
        }
        ##
        ...
        if np.all(spin >= 0):
            if sign_p >= 0:
                filename = dir_path + "/data/bcw_qnm_tables/l%s/n%sl%sm%s.dat" % (
                    l,
                    n + 1,
                    l,
                    m,
                ...
                filename = dir_path + "/data/bcw_qnm_tables/l%s/n%sl%smm%s.dat" % (
                    l,
                    n + 1,
                    l,
                    m,
            ...
            try:
                ...
                raise ValueError(
                    "Tabulated values for the (%s,%s,%s) mode are not stored and cannot be interpolated"
                    % mode
                )
            f = (
                sign_p
                * sign_m
                * np.interp(np.abs(spin), chi, omega_re)
                / mass
                / 2
                / np.pi
            )
...
def spherical_spheroidal_mixing(
    lm,
    mode,
    spin,
    method="BK14",
    ...
    #####
        ...
        if np.mean(spin < 0) and np.mean(spin < 0) != 1:
            ...
            mu_re[mask], mu_im[mask] = spherical_spheroidal_mixing(
                lm,
                mode,
                spin[mask],
                s=s,
                ...
                method=method,
                qnm_method=qnm_method,
                enforce_sxs_convention=enforce_sxs_convention,
            )
            mu_re[~mask], mu_im[~mask] = spherical_spheroidal_mixing(
                lm,
                mode,
        ...
        if mlln not in allowed_keys:
            raise KeyError(
                "(%s,%s,%s,%s) mode is not allowed by this method. Allowed modes are: "
                % mlln
                + str(allowed_keys)
            )
        p1_re, p2_re, p3_re, p4_re, p1_im, p2_im, p3_im, p4_im = _fitting_coeffs[mlln]
        mu_re = (
            p1_re * abs(spin) ** p2_re
            + p3_re * abs(spin) ** p4_re
            + 1
            - np.sign(abs(lt - l))
        )
...
def chi_s_from_spins(q,chi1z,chi2z):
    ...
    ----------
    chi1z : float
        z-component of the dimensionless spin of the primary black hole.
    chi2z : float
